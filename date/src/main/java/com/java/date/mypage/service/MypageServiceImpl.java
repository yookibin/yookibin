...
public class MypageServiceImpl implements MypageService {
	...
	public List<LoadCourseDto> getMyCourse(String id, String like) {
		...
		for (int i = 0; i < sList.size(); i++) {
			...
			dto.setSave_cplace1(placeDao
					.getInfo(sList.get(i).getSave_cplace1()));
			dto.setSave_cplace2(placeDao
					.getInfo(sList.get(i).getSave_cplace2()));
	...
	/*
	 * @Override public void pointCheck(ModelAndView mav) { Map<String,Object>
	 * hMap = mav.getModelMap(); HttpServletRequest request=
	 * (HttpServletRequest)hMap.get("request");
	 * 
	 * String id=request.getParameter("id");
	 * 
	 * logger.info("id"+id); List <MypageDto> mypageDto=mypagedao.point(id);
	 * 
	 * mav.addObject("mypageDto",mypageDto); mav.setViewName("mypage/point");
	 * 
	 * }
	 * 
	 * @Override public void usingevent(ModelAndView mav) { Map<String,Object>
	 * hMap =mav.getModelMap(); HttpServletRequest
	 * request=(HttpServletRequest)hMap.get("request");
	 * 
	 * String id=request.getParameter("id");
	 * 
	 * logger.info("id"+id); List <MypageDto> eventDto=mypagedao.usingevent(id);
	 * 
	 * mav.addObject("eventDto",eventDto); mav.setViewName("mypage/usingevent");
	 * 
	 * }
	 */
